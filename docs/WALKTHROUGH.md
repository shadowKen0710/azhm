...
- 打开 `/` 是**对外落地/定价页**（公开），CTA 通向照护者登录与患者大屏。
- 顶部 **角色切换**：`照护者 ⇄ 患者`。
...
## 〇、对外落地/定价页 `/`
面向照护者的营销页：情感 hero（家人 AI 声线织入共同回忆的「记忆气泡」）+ 核心功能 + **透明计价**（接真实 `billingApi`：token 成本×1.6、算力点、充值套餐、支付方式）+ 伦理信任条 + CTA。公开无需登录。

![落地/定价页](screens/landing.png)
